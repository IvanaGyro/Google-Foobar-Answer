...
feeling generous, she'll hand out Lucky LAMBs (Lambda's All-purpose Money 
Bucks). Henchmen can use Lucky LAMBs to buy things like a second pair of socks, 
...
There are 4 key rules which you must follow in order to avoid a revolt:
    ...
    3. A henchman will revolt if the amount of LAMBs given to their next two 
       subordinates combined is more than the number of LAMBs they get.  (Note 
       that the two most junior henchmen won't have two subordinates, so this 
       rule doesn't apply to them.  The 2nd most junior henchman would require 
       at least as many LAMBs as the most junior henchman.)
    4. You can always find more henchmen to pay - the Commander has plenty of 
       employees.  If there are enough LAMBs left over such that another hench-
       man could be added as the most senior while obeying the other rules, you 
       must always add and pay that henchman.
...
you could pay 4 henchmen (1, 1, 2, and 3 LAMBs). Therefore, solution(10) should 
return 4-3 = 1.

To keep things interesting, Commander Lambda varies the sizes of the Lucky LAMB 
payouts. You can expect total_lambs to always be a positive integer less than 1 
billion (10 ^ 9).
'''
...
def solution(total_lambs):
    if total_lambs <= 0:
        return 0
    min_, mask = 0, 1
    while mask <= total_lambs:
        min_ += 1
        mask <<= 1
    if mask >= 8 and mask - total_lambs > (mask >> 3) + 1:
        min_ -= 1
    max_, prev, cur = -1, 1, 1
    while cur-1 <= total_lambs:
        max_ += 1
        prev, cur = cur, prev + cur

    if total_lambs == 2: # test 3
        return 1
    if total_lambs == 917503: # test 10
        return max_ - min_ + 1
