...
planted her space station in the middle of a quasar quantum flux field. In 
...
noisy).  We can examine which grids are equivalent by swapping rows and columns.
...
def cycle(c):
    ...
    for a, m in cnt.items():
        res *= factorial(n)
        n -= a * m
        res //= factorial(n) * a ** m * factorial(m)
    return res

def solution(w, h, s):
    return str(sum(
        s ** sum(gcd(wc, hc) for wc in wp for hc in hp) * cycle(wp) * cycle(hp)
        for wp in partition(w) 
        for hp in partition(h) 
